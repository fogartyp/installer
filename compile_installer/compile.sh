...
proto="http"
ansible_url="releases.ansible.com/ansible"
ansible_bin="ansible-latest.tar.gz"
...
function get_ansible() {
  info
  info "Getting latest Ansible binary"
  # Check $CWD for file
  if stat --format=%s ansible.tar.gz > /dev/null 2>&1; then
    info "File exists. Checking file integrity"
    info
    # List contents of archive to prove integrity, else delete archive
    if tar -tzf ansible.tar.gz > /dev/null 2>&1; then
      info "Integrity check successful"
      info
      exit
    else
      error
      error "Integrity check failed. Removing file"
      error "Run again to get new file or change source of file"
      error
      rm -rf ansible.tar.gz
    fi
  # Download the ansible tarball
  elif curl --output /dev/null --silent --head --fail "${proto}://${ansible_url}/${ansible_bin}"; then
    info
    info "Starting download"
    info
    curl -v -L -o ansible.tar.gz ${proto}://${ansible_url}/${ansible_bin}
  else
    error
    error "URL does not exist. Check URL and try again"
    error
  fi
}
